...
export async function checkExtensionPackageUpdates(workspaceId?: string): Promise<boolean> {
  ...
  try {
    ...
    publishNotification({
      level: "info",
      title: "Extension 更新检查完成",
      message: result.total === 0 ? "当前没有可用更新。" : `发现 ${result.total} 个可用更新。`
    });
    return true;
  } catch (error) {
    return reportFailure(target.id, "无法检查 Extension 更新", error);
  }
}
...
export async function installExtensionPackage(
  ...
  return mutate("extension.package.install", { source, scope }, scope, workspaceId, "Extension 已安装");
}
...
export async function updateExtensionPackage(
  ...
  return mutate("extension.package.update", { source, scope }, scope, workspaceId, "Extension 已更新");
}
...
async function mutate<T extends MutationType>(
  ...
  try {
    ...
    return reportFailure(target.id, "Extension 操作失败", error);
  }
}
