...
const SESSIONS: SessionSummary[] = [
  { fileIdentity: "session-file-live", id: "session-live", path: "/sessions/live.jsonl", cwd: "/work", name: "修复恢复协议", nameSource: "explicit", modifiedAt: 300, messageCount: 12 },
  { fileIdentity: "session-file-recent", id: "session-recent", path: "/sessions/recent.jsonl", cwd: "/work", name: "性能审计", nameSource: "explicit", modifiedAt: 200, messageCount: 8 },
  { fileIdentity: "session-file-old", id: "session-old", path: "/sessions/old.jsonl", cwd: "/work", name: "旧会话", nameSource: "explicit", modifiedAt: 100, messageCount: 4 }
];
...
function operation(overrides: Partial<OperationView> = {}): OperationView {
  return {
    ...
    sessionFileIdentity: "session-file-live",
    sessionGeneration: 1,
    startedAt: 1,
    ...overrides
  };
...
describe("session navigation projection", () => {
  it("groups the real active operation separately from recent sessions", () => {
    const rows = buildSessionNavigationRows({
      sessions: SESSIONS,
      activeFileIdentity: "session-file-live",
  ...
  it("distinguishes waiting input from running and does not present terminal operations as live", () => {
    const waitingRows = buildSessionNavigationRows({
      sessions: SESSIONS,
      activeFileIdentity: "session-file-live",
      operation: operation({ lifecycle: "waiting-input" })
    });
  ...
  it("preserves server ordering and defensively removes duplicate physical identities", () => {
