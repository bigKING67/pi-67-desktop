...
6. **Native-first, independently checked** — the current CLI implements; L2 adds one cross-provider Channel review
...
python3 ./.trellis/scripts/task.py set-meta <name> risk_level <L1|L2|high>
python3 ./.trellis/scripts/task.py set-meta <name> execution_mode <inline|native|channel>
python3 ./.trellis/scripts/task.py set-meta <name> review_mode <native|channel>
python3 ./.trellis/scripts/task.py set-meta <name> handoff_mode <none|relay>
...
### Relay and Channel System

Pi-67 uses two distinct Channel classes:

- `relay-<createdAt>-<task.id>` is a durable, project-scoped, metadata-only
  handoff log with no worker. It supplements `handoff.md`; Git and task
  artifacts remain authoritative.
- `impl-<task.id>-r<N>` / `check-<task.id>-r<N>` are ephemeral, one-worker
  execution logs. The main session must stop overlapping edits while a worker
  runs, inspect raw events and the resulting diff, then ensure the worker exits.

Use `python3 ./.trellis/scripts/trellis_relay.py --help` for Relay operations.
Normal cross-CLI handoff never spawns a worker. Channel workers support only
Claude/Codex; Pi/Grok participate through their interactive CLI and explicit
task paths.
...
- L0 conversation or low-risk micro work stays direct without a Trellis task.
- L1/L2 work creates a Trellis task by default under the project's standing
  authorization. The user may opt out for a specific task.
- Task creation is not implementation approval. Planning still happens first,
  and complex work requires reviewed `prd.md`, `design.md`, and `implement.md`.
- L1 defaults to `native/native/relay`; L2 defaults to
  `native/channel/relay`. `execution_mode=channel` requires an explicit user
  request.
...
- `handoff.md` — bounded latest-state snapshot for sequential CLI takeover; detailed evidence stays under `research/`.
...
No active task. Classify the turn before acting: L0 stays direct; L1/L2 creates a Trellis planning task under the project's standing authorization unless the user opts out.
Task creation is not implementation approval. Complex work must complete and review `prd.md`, `design.md`, and `implement.md` before `task.py start`.
[/workflow-state:no_task]
...
Set task metadata explicitly. L1 defaults to native/native/relay; L2 defaults to native/channel/relay; Channel implementation is explicit-only.
...
Flow: native implementation -> task-selected review (`channel` for L2 by default) -> `trellis-update-spec` -> commit (Phase 3.4) -> `/trellis:finish-work`.
Read `task.json.meta` before dispatch. `execution_mode=native` uses the current platform or its native sub-agent; `execution_mode=channel` is allowed only after an explicit user request. `review_mode=channel` spawns exactly one cross-provider Channel check worker. The main session stops overlapping edits, reads raw Channel events and the final diff, owns judgment, and ensures the worker exits.
...
Flow: `trellis-before-dev` -> edit -> task-selected review -> validation -> `trellis-update-spec` -> commit (Phase 3.4) -> `/trellis:finish-work`.
Do not dispatch implement/check sub-agents in inline mode.
If task metadata requires Channel review, the main session may dispatch one cross-provider Channel check worker after inline implementation; this does not change `execution_mode=inline`.
...
- `in_progress` implementation -> follow `execution_mode`; native is default.
- `in_progress` check -> follow `review_mode`; L2 defaults to one cross-provider Channel check worker.
- Cross-CLI continue/takeover -> `trellis_relay.py resume`; ordinary Relay never spawns a worker.
...
- Planning or unclear requirements -> `trellis-brainstorm`.
- Before editing -> `trellis-before-dev`; after editing -> follow task `review_mode`.
- Cross-CLI continue/takeover -> `trellis_relay.py resume`; ordinary Relay never spawns a worker.
- Repeated debugging -> `trellis-break-loop`; spec updates -> `trellis-update-spec`.
...
- At most one platform main session owns edits and at most one Channel worker may be live. Never edit overlapping scope while a worker runs.
- Do not silently switch Channel Provider. Claude/Codex are the only Channel Worker providers; Pi/Grok remain interactive participants.
...
Goal: classify the request, create a task for L1/L2 under the project's standing authorization unless the user opts out, and produce the planning artifacts required before implementation.

#### 1.0 Create task `[required · once]`

Create the task directory for L1/L2 after classification (or after explicit consent when the user previously opted out). The command sets status to `planning`, writes `task.json`, creates a default `prd.md`, and auto-targets the new task when session identity is available:
...
Set explicit routing metadata during planning. L1 defaults to
`risk_level=L1`, `execution_mode=native`, `review_mode=native`,
`handoff_mode=relay`; L2 defaults to `L2/native/channel/relay`.
...
Read `task.json.meta.execution_mode` first:

- `native` (default): use the current platform main session or its native
  `trellis-implement` sub-agent as described below.
- `inline`: the main session loads `trellis-before-dev` and implements directly.
- `channel`: only after an explicit user request, create one ephemeral
  `impl-<task.id>-r<N>` Channel and spawn one Claude/Codex implement worker.

For Channel implementation, pass the Provider explicitly, use a bounded
timeout, pass Codex `--sandbox danger-full-access`, stop main-session edits in
the worker's scope, and read raw events plus the resulting diff before accepting
the work. Never silently fall back to another Provider.
...
Read `task.json.meta.review_mode` first.

For `review_mode=native`, dispatch the current platform's native
`trellis-check` agent (or load the `trellis-check` skill in inline-only hosts).
The prompt starts with `Active task: <task path>` and tells the child it is
already the check agent and must not recurse.

For `review_mode=channel` (the L2 default), the main session:

1. Stops editing the declared review scope and snapshots Git/dirty state.
2. Creates one ephemeral `check-<task.id>-r<N>` Channel.
3. Selects a different Provider when available: Codex main -> Claude reviewer;
   Claude main -> Codex reviewer; Pi/Grok main -> Claude reviewer.
4. Spawns exactly one worker with `check.jsonl`, task artifacts, explicit
   editable/forbidden scope, validation commands, and timeout. Codex workers
   always receive `--sandbox danger-full-access`.
5. Waits for `done,error,killed`, reads `messages --raw`, re-reads the final
   diff, and re-runs decisive checks. Do not use a custom `--tag` as completion.
6. Allows small mechanical self-fixes, but keeps design/contract changes as
   report-only findings for the main session.
7. Terminates the worker and confirms no Supervisor remains.

No silent Provider fallback is allowed. If Channel review cannot run, report
the exact blocker and mark the gate degraded. High-risk tasks remain blocked
until independent review succeeds or the user explicitly waives it.
