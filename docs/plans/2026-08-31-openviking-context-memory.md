...
Status: All sealed OpenViking review findings are source-, real-Lab-, and macOS-packaged-verified; Windows and production acceptance remain pending
Owner: Codex
Started: 2026-08-31
...
- Loaded Sessions apply stricter privacy settings monotonically at the next Pi
  lifecycle or OpenViking Tool boundary; no compatibility path may remove actor
  scope, and no candidate may pass while retaining a credential-like value.
...
| --- | --- | --- | --- |
| OBSERVED | Desktop is at pre-delivery baseline `47f25710e7d78ab4996103d3ee0f5550327b7f7b`, eight commits ahead of `origin/main`, with the completed scoped OpenViking sealed-review remediation still uncommitted; Protocol version is v4. | live Git and source | 2026-09-04 |
...
| VERIFIED | Review Craft identified three P0 OpenViking boundaries: loaded-Session privacy split-brain, actor-scope compatibility widening, and label-only credential redaction. The remediation now enforces monotonic privacy tightening, actor-scope fail-closed Recall, and whole-value redaction plus residual scanning. | sealed review plus focused and aggregate source gates | 2026-09-04 |
...
| Privacy changes in a loaded Session are monotonic; actor scope never downgrades; candidate redaction replaces whole credential expressions and rejects residual credential-like values. | A running Session must be able to lose authority without silently gaining it, and compatibility or review UX must never widen a privacy boundary. | Pi/OpenViking later provide an equivalent atomic policy-reload and scope contract with independently verified guarantees. |
...
- [x] 16. Remediate the three P0 findings from the 2026-09-04 OpenViking review.
  Re-read privacy at Pi lifecycle and Tool boundaries without permitting a loaded
  Session to widen authority; fail closed when a server rejects `peer_scope`;
  replace complete credential-bearing expressions and reject residual high-risk
  values before `redactionStatus=passed`. Pin Extension `0.2.0-desktop.4` in
  capability catalog `2026.09.04.2`, then pass focused, aggregate, and fresh
  macOS packaged smoke gates.
- [x] 17. Close the remaining sealed-review findings in Extension
  `0.2.0-desktop.6`: cancel and generation-fence Recall, persist branch-aware
  capture lineage, add stable source-message identity and verified outbox replay,
  authorize every private Tool URI, bound current-Session Archive expansion,
  unify the recoverable connection state, ignore retired on-disk owners unless
  configured, and select credentials from one endpoint-matched source. Pin the
  resulting tree in capability catalog `2026.09.04.4`, then pass focused,
  provenance, aggregate, real v0.4.16 compatibility, packaged isolation,
  Compaction fallback, and fresh macOS preview gates.
...
| Desktop host/renderer | Agent Host/config/client/router, secure-credential store, Supervisor, candidate assembler/governance, renderer review, authority, binding recovery, isolation, and Compaction fallback tests | no direct renderer service channel; token/private URI/raw Session identity never reaches Renderer or Gateway; states render and recover correctly | PASS: exact Commit provenance, bounded Pi JSONL hashing, OpenViking task/diff reconciliation, whole-value credential redaction plus residual scanning, durable private candidates, explicit review, separate enterprise submission, and submitted-not-shared behavior are covered. Enterprise credential restoration no longer gates Agent Host readiness, and an empty Profile does not probe Keychain. The latest aggregate source gate passes with 669 test files passed, 2 skipped, and 3,414 tests passed; 5 tests are contractually skipped. Architecture reports 944 modules, 3,515 imports, and zero cycles; Statements/Branches/Functions/Lines coverage is 82.12%/76.15%/86.18%/86.00%. |
...
| Packaged artifact | unsigned macOS arm64 package, product flow, isolation receipt, and native Compaction-fallback receipt | correct renderer/host wiring; no OM runtime; no credential leakage or local-owner contamination; Pi fallback continuity | PASS on macOS arm64. The final preview packages OpenViking `0.2.0-desktop.6` with source-tree SHA-256 `08c076cc…` in capability catalog `2026.09.04.4`; the prepared package contains the complete checked runtime-module closure and has tree SHA-256 `a6638415…`. Packaged Electron smoke passes, opens `app://pi67`, and binds `app.asar` SHA-256 `b0ff42e9c4d4d0223e8f5aaeea471c9e81ef504cb71beda59988408316b753cf`. Fresh serial exact-source isolation receipt `0762da9b-ef1f-4096-937e-979929023844` records one isolated Session, 16 synthetic OpenViking requests, zero canonical Session mutations, zero existing-memory injection, and complete cleanup. Fresh serial Compaction receipt `a4ba574e-588e-458e-a890-1bf3f031634f` records six failed OpenViking connections, exactly one Pi-native threshold Compaction with `fromExtension=false`, a continued Turn, and same-Session Resume after restart. The artifact remains present with the same hash after both serial receipts and the preview process remains live. Earlier deep authenticated product E2E remains bound to executable SHA-256 `79019361f697c1a81489dba3e94631b0977770c1ab15236f1f033f9de6238874`: Device Authorization, Workspace binding/recovery, real model-selected shared search/read, XML escaping, untrusted projection, Tool-authority non-inheritance, revoke-to-zero, credential expiry, Host epoch recovery, and service degradation all pass. |
...
- 2026-09-04: Remediated the three P0 findings from the sealed OpenViking review.
  Loaded Sessions now apply only privacy reductions at lifecycle/Tool boundaries;
  `peer_scope` incompatibility skips Recall without an unscoped retry or raw
  fallback; Experience review removes complete synthetic credential expressions
  and rejects suspicious residual values. Focused tests pass 18/18, the aggregate
  gate passes 661 files and 3,377 tests, and a fresh catalog `2026.09.04.2`
  macOS arm64 preview passed packaged smoke and opened with `app.asar` SHA-256
  `068867159a0f5bd7ef4f2aedfce1e09e3584ac1b0592788e7b6d49b3023e2d29`.
- 2026-09-04: Closed all remaining sealed-review findings in OpenViking Extension
  `0.2.0-desktop.6`. Recall is abortable and Session-generation fenced; Capture
  persists semantic branch lineage and stable source-message identities; the
  outbox verifies uncertain remote acceptance before replay; private Tools are
  limited to the current actor/Workspace; Archive expansion is current-Session,
  bounded, escaped, and cancellable; connection recovery uses one state; owner
  conflicts inspect configured packages rather than retired directories; and
  credentials come from exactly one endpoint-matched source. Real v0.4.16
  compatibility passes without reading existing resources and deletes its exact
  synthetic Session. Packaged testing exposed and fixed an independent capability
  preparation defect: four new runtime modules were omitted by the old copy
  allowlist. Capability preparation now validates the recursive local-module
  closure before packaging. Final catalog `2026.09.04.4`, aggregate 669-file /
  3,414-test gate, exact-source packaged smoke, isolation receipt, and Pi-native
  Compaction/Resume receipt all pass; the fresh preview is open.
