...
- 2026-08-28: Source freeze `8ddd2f0...` was committed, pushed, and rebuilt cleanly on macOS. Windows run `33154565426` attempt 1 passed provenance, packaging, packaged smoke, UI, candidate-byte, and Alpha.36 baseline gates, then failed the existing-profile lifecycle because its verifier still required an Extension-scoped model switch to overwrite global Pi settings. The Pi 0.84.3 runtime contract and its targeted test require those settings bytes to remain unchanged, so attempt 1 and its macOS sibling are superseded pending a scoped verifier-contract fix and fresh exact-SHA two-platform rebuild.
- 2026-08-28: Replacement source `c17baec...` was committed, pushed, and rebuilt cleanly on macOS. Windows run `33156183812` attempt 1 passed provenance and Windows packaging, then failed before installer certification because the helper test's full-table `Win32_Process` CIM enumeration exceeded its 15-second command timeout. No testable Windows candidate was emitted. Narrow process probes by exact executable path before querying candidate command lines, then rebuild both platforms again from the replacement SHA.
