...
- 内置 `Groland` 是一个 Provider、一个 Credential、七个图片+推理模型：五个 Claude 走 Anthropic Messages/x-api-key，两个 GPT 走 OpenAI Responses/Bearer。DeepSeek 继续走 Pi 官方 Provider，其官方目录中的所有模型都声明原生搜索；它们复用同一个 Pi API Key 调用官方 Responses `/responses`，并接收 `in_progress` / `searching` / `completed` 流式搜索状态。自定义或第三方 DeepSeek Endpoint 不继承此能力
