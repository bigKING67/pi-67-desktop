...
1. **分别确定可见集合与编辑权限**。只有用户明确要求“仅这些视图可见”等可见性限制，才设置 `all_visible = false`；`visible_views` 使用已确认的可见视图名称（非 viewID），不可为空。仅提及“甘特图不可编辑”不构成可见性限制。
2. 未要求改变可见性时，读取并保留已有 `visibility`；新建权限且没有视图可见性限制时使用 `all_visible = true`。若用户要求隐藏部分视图，先取得完整视图列表计算剩余集合；范围不明或集合为空而当前合同不支持时澄清，不提交非法空 allowlist。
3. `allow_edit` 与 `visibility` 独立。新建表权限为 `edit` 且没有编辑限制时默认 `true`；明确禁止编辑视图时为 `false`。修改现有权限未涉及编辑时保留原值。当前 `allow_edit` 是整体视图编辑开关，不能用可见性替代单个视图编辑权限；无法表达用户的逐视图差异时说明限制并澄清。无论其值如何，都必须包含已确认的 `visibility`。
