...
function codingQuotas(limit, experienceLimit, sharedExperienceLimit) {
  const quotas = scaleQuotas(limit, CODING_QUOTA_WEIGHTS);
  if (Number.isFinite(Number(experienceLimit))) {
    quotas.experiences = Math.max(0, Math.min(3, Math.floor(Number(experienceLimit))));
  }
  if (Number.isFinite(Number(sharedExperienceLimit))) {
    quotas.resources = Math.max(0, Math.min(5, Math.floor(Number(sharedExperienceLimit))));
  }
  return quotas;
}
...
export function buildContextSearchBody(cfg = {}, options = {}) {
  ...
  if (limitConfigured || Number.isFinite(Number(cfg.experienceRecallLimit))) {
    body.quotas = codingQuotas(limit, cfg.experienceRecallLimit, cfg.sharedExperienceLimit);
  }
...
async function buildFallbackInjectionBlock(fetchJSON, items, cfg, actorPeerId = "", log = () => {}) {
  ...
  const lines = [
    "<openviking-context>",
    "Relevant context from OpenViking. Use viking_read to expand only a selected URI.",
  ];
  let contentCount = 0;
  let hintCount = 0;
...
function wrapContext(body) {
  return [
    "<openviking-context>",
    "Relevant memory from OpenViking. Use viking_search and viking_read only when more detail is needed.",
    body,
    "</openviking-context>",
  ].join("\n");
}
