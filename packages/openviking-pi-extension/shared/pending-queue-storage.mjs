...
import { operationPriority } from "./pending-replay-policy.mjs";
...
function getPendingDir(context) {
  return context?.directory || process.env.OPENVIKING_PENDING_DIR || DEFAULT_PENDING_DIR();
}
...
export function getReplayLimit() {
...
async function findExistingByDedupKey(dir, dedupKey, context) {
  ...
  for (const f of files) {
    ...
    try {
      ...
      if ((!context || entry?.scopeKey === context.scopeKey) && entry?.dedupKey === dedupKey) return { filename: f, entry };
    } catch {
...
async function recoverStaleProcessing(dir, context) {
  let files;
  try {
    files = await readdir(dir);
  } catch {
  ...
  for (const f of files) {
    ...
    try {
      if (!await ownedEntry(dir, f, context)) continue;
 ...
 */
export async function enqueue(type, sessionId, payload, options = {}, context) {
  const dir = getPendingDir(context);
  const now = Number.isFinite(options.createdAt) ? options.createdAt : Date.now();
  const dedupKey = makeDedupKey(type, sessionId, context ? { scopeKey: context.scopeKey, payload } : payload);
  ...
  const entry = {
    ...
    retries: 0,
    dedupKey,
    ...(context ? { scopeKey: context.scopeKey } : {}),
  };
  ...
  const existing = await findExistingByDedupKey(dir, dedupKey, context);
  if (existing) {
  ...
  const duplicate = await findExistingByDedupKey(dir, dedupKey, context);
  if (duplicate) {
...
export async function listPending(context) {
  ...
  let files;
  try {
    await recoverStaleProcessing(dir, context);
    files = await readdir(dir);
  } catch {
    return [];
  }
 ...
 */
export async function claimForReplay(filename, context) {
  if (!filename.endsWith(".json")) return null;
  const dir = getPendingDir(context);
  if (!await ownedEntry(dir, filename, context)) return null;
...
export async function releaseReplayClaim(filename, context) {
  if (!filename.endsWith(".processing")) return false;
  const dir = getPendingDir(context);
  if (!await ownedEntry(dir, filename, context)) return false;
  try {
 ...
 */
export async function dequeue(filename, context) {
  const dir = getPendingDir(context);
  if (!await ownedEntry(dir, filename, context)) return false;
  try {
 ...
 */
export async function incrementRetry(filename, entry, context) {
  const dir = getPendingDir(context);
  if (!await ownedEntry(dir, filename, context)) return false;
 ...
 */
export async function cleanStale(context) {
