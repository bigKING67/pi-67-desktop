...
- **Dispatcher wait pattern**: use `--kind done` / `--kind turn_finished`
  (Trellis-emitted system events). Trellis 0.6.15 has no `--tag` flag on
  `send`, `wait`, or `messages`; inspect terminal events with `--raw`.
  See `references/command-reference.md` "tag vs kind".
