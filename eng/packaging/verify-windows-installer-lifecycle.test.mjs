...
describe("Windows installer lifecycle contract", () => {
  ...
  it("enforces bounded real-user Provider, Catalog, create and restart gates", async () => {
    ...
    expect(source).toContain("settingsAccessibleName: initialUserInterfaceContract.settingsAccessibleName");
    ...
    expect(source).toContain("requiredCapabilityPaths: [...initialRuntimeAssetContract.requiredCapabilityPaths]");
    ...
    expect(processSource).toContain("WINDOWS_TIMEOUT_DIAGNOSTIC_LEAD_MS = 30_000");
    expect(source).toContain("report.windowsInstallerFailure = error.windowsInstallerEvidence");
