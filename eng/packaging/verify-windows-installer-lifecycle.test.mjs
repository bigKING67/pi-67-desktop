...
describe("Windows installer lifecycle contract", () => {
  ...
  it("shows liveness for unattended updates without exposing the assisted wizard", async () => {
    ...
    expect(installer).toMatch(/!macro customInit\s+StrCpy \$Pi67UpdateProgressVisible "0"\s+\$\{If\} \$\{isUpdated\}\s+\$\{AndIf\} \$\{Silent\}[\s\S]*?SpiderBanner::Show \/MODERN[\s\S]*?StrCpy \$Pi67UpdateProgressVisible "1"/u);
