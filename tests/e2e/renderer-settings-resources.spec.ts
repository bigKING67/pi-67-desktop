...
test("separates extension packages, extensions, skills, prompt templates, and context rules", async ({ page }) => {
  ...
  await expect(globalPanel.getByText("1 个技能 · 内置基线 1.0.2", { exact: true })).toBeVisible();
  ...
  await expect(suiteDetail.getByText("1.0.2", { exact: true })).toBeVisible();
  await expect(suiteDetail.getByText("可独立管理", { exact: true })).toBeVisible();
  await expect(suiteDetail.getByText("https://github.com/xbtlin/ai-berkshire", { exact: true })).toBeVisible();
  await expect(suiteDetail.getByText("fad8a0fae", { exact: true })).toBeVisible();
  await suiteDetail.getByRole("button", { name: "返回全局可用技能" }).click();
  ...
  await expect(suiteDetail.getByText("1.0.3 · Overlay", { exact: true })).toBeVisible();
  await expect(suiteDetail.getByText("已是最新", { exact: true })).toBeVisible();
  await expect(suiteDetail.getByText("1 个技能 · 当前 1.0.3 · 最新兼容 1.0.3", { exact: true })).toBeVisible();
  ...
  await expect(suiteDetail.getByText("1.0.2 · 内置", { exact: true })).toBeVisible();
  await suiteDetail.getByRole("button", { name: "返回全局可用技能" }).click();
