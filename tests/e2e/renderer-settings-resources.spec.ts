...
test("separates extension packages, extensions, skills, prompt templates, and context rules", async ({ page }) => {
  ...
  await expect(globalPanel.getByText("2 个技能 · 内置基线 0.5.0", { exact: true })).toBeVisible();
  ...
  await expect(suiteDetail.getByText("1 个技能 · 当前 1.0.7 · 最新兼容 1.0.7", { exact: true })).toBeVisible();
